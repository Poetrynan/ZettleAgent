...
npm run tauri build  # Release 설치 패키지 (build:prod로 빌드 시 모델 1회 준비)
```

대용량 에셋(임베딩 모델, ORT WASM, 폰트)은 git 저장소에 **포함되지 않습니다**. `tauri build`가 설치 패키지용으로 자동 준비합니다. 최종 사용자는 이 단계를 실행하지 않습니다.
