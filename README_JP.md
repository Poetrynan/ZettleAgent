...
npm run tauri build  # Release インストーラー（build:prod でビルド時にモデルを一度取得）
```

大きなアセット（埋め込みモデル、ORT WASM、フォント）は git リポジトリに**含まれません**。`tauri build` がインストーラー用に自動準備します。エンドユーザーはこの手順を実行しません。
